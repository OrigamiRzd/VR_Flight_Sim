...
namespace UnityStandardAssets.Vehicles.Aeroplane
{
    ...
    public class AeroplaneController : MonoBehaviour
    {
        ...
        //Calculations
        public float metersPerSecondToKnots = 1.94384f;
        public float metersPerSecondToFtPerMin = 196.85f;
        public float metersToFt = 3.28084f;
        ...
        private float[] lastAltitudes = { 0, 0 };
        private float lastAltitudeIndex = -1;
        private int missedCount = 1;
        private int displayIntervalCount = 0;
        ...
        public GameObject YokeStem_L;
        private Vector3 yokeStartL;
        public GameObject YokeStem_R;
        private Vector3 yokeStartR;
        public GameObject ThrottleCont;
        private void Start()
        {
            yokeStartL = YokeStem_L.transform.localPosition;
            yokeStartR = YokeStem_R.transform.localPosition;
        ...
        void updateYoke(float roll, float pitch)
        {
            float rollspot = -(roll * 45);
            YokeStem_L.transform.localEulerAngles = new Vector3(0f, rollspot, 0f);
            YokeStem_R.transform.localEulerAngles = new Vector3(0f, rollspot, 0f);
            float pitchspot = (.1053f * pitch);
            YokeStem_L.transform.localPosition = new Vector3(yokeStartL.x, yokeStartL.y + pitchspot, yokeStartL.z);
            YokeStem_R.transform.localPosition = new Vector3(yokeStartR.x, yokeStartR.y + pitchspot, yokeStartR.z);
        }

        void updateThrottle(float throttle)
        {
            float newspot = ((throttle-1f)*25f)-5f;
            ThrottleCont.transform.localEulerAngles = new Vector3(0f, -90f, newspot);
        ...
        public void Move(float rollInput, float pitchInput, float yawInput, float throttleInput, bool airBrakes)
        {
            ...
            updateYoke(rollInput, pitchInput);
        ...
        {
            // update variables
            fSpeed = m_Rigidbody.transform.InverseTransformDirection(m_Rigidbody.velocity).z;
            airspeed = (fSpeed * metersPerSecondToKnots);
            //		verticalspeed = (baseRB.velocity.y * metersPerSecondToFtPerMin);
            Altitude = (m_Rigidbody.transform.position.y * metersToFt) - 5.57743f;

            if (displayIntervalCount > 10)
            {
                if (lastAltitudes[(int)(lastAltitudeIndex / -2 + 0.5f)] != Altitude)
                {
                    lastAltitudes[(int)(lastAltitudeIndex / 2 + 0.5f)] = Altitude;
                    float thisAlt = lastAltitudes[(int)(lastAltitudeIndex / 2 + 0.5f)];
                    lastAltitudeIndex *= -1;
                    float lastAlt = lastAltitudes[(int)(lastAltitudeIndex / 2 + 0.5f)];
                    VerticalSpeed = (thisAlt - lastAlt) * 60 / (Time.deltaTime * missedCount);
                    missedCount = 1;
                }
                else
                {
        ...
        public float Heading
        {
            get
            {
                if (m_Rigidbody != null)
                {
                    return m_Rigidbody.rotation.eulerAngles.y;
                }
                else
                {
                    return 0;
        ...
        public float Airspeed
        {
            ...
            {
                this.airspeed = value;
                Vector3 newVel = m_Rigidbody.transform.forward * value / metersPerSecondToKnots;
